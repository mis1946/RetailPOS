...
Public Class frmMain
    ...
    Private Sub newOrder()
        ...
        lblMaster80.Text = p_oTrans.POSNumber
...
#Region "Form Events"
    Private Sub Form_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        If pnLoadx = 0 Then
            ...
            Else
                lblPStat.Visible = False
            End If

            Dim row As DataRow
            txtDetail00.AutoCompleteCustomSource.Clear()
            For Each row In p_oTrans.SearchItem.Rows
                txtDetail00.AutoCompleteCustomSource.Add(row.Item("sDescript").ToString())
            Next
    ...
    Private Sub Form_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Select Case e.KeyCode
            Case Keys.Escape
                ...
                If MsgBox("You are going to Log out. Continue?",
        ...
        With p_oTrans
            Select Case lnIndex
                ...
                Case 3 'Merge Order
                    ...
                    If .HasDiscount Then
                        MsgBox("Unable to merge/split transaction with discount", _
                ...
                Case 8 'issue discount
                    If DataGridView1.Rows(0).Cells(1).Value <> "" Then
                        If IFNull(p_oTrans.Master("sMergeIDx"), "") <> "" Or .WasSplitted Then
                            MsgBox("Unable to issue discount on Merge/Split Transaction", _
                                      MsgBoxStyle.Critical, _
                                      "Warning")
                            Call initPanel(1, True)
                            Exit Sub
                ...
                Case 12 'print bill
                    ...
                    End If
                Case 13 'end shift
                    If MsgBox("You are going to end your shift. Continue?",
                              MsgBoxStyle.Question & MsgBoxStyle.YesNo,
                              "Confirm") = MsgBoxResult.Yes Then
                ...
                Case 14 'cancel receipt
                    .CancelOR()
                Case 15 'pay charge
                    '.PayChargeWithReceipt()
                Case 16
                    'Call newOrder()
                    'If MsgBox("Create New Order?",
                ...
                Case 19 'Item Search
                    ...
                    If p_oTrans.ItemLookUp(txtDetail00.Text) Then loadDetail()
                    'End If
                Case 20 'showtable
                    If p_oTrans.showTable() Then
                        loadMaster()
                        loadDetail()

                        Select Case p_oTrans.Master("sTableNox")
                            Case 1 To 12
                                pnTablePage = 0
                            Case 13 To 24
                                pnTablePage = 1
...
endProc:
    ...
    Private Sub lblTable_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If p_oTrans.ManageTable(CInt(loLbl.Text)) Then
            Call newOrder()
            showComputation()
        End If
    End Sub
            ...
            If loDT.Rows.Count > 0 Then
                For lnCtr = 0 To loDT.Rows.Count - 1
                    If lnCtr < pxeMaxDtail Then
                        ...
                        loPic.BackgroundImage = Nothing
                        loPic.BorderStyle = BorderStyle.FixedSingle
                        loPic.Tag = loDT(lnCtr)("sDescript")
...
#End Region
    ...
    Private Function checkMergeOrder() As Boolean
        Dim loFrm As frmDiscountMerger
        loFrm = New frmDiscountMerger
        loFrm.ShowDialog()

        checkMergeOrder = loFrm.Cancelled

        Return checkMergeOrder
    End Function
